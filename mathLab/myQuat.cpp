...
MyQuat::MyQuat(float angleDeg, Vector &axis)
{
...
MyQuat MyQuat::multiplyBy(const MyQuat &other) const
{
	...
	Vector v1 = this->v.getCrossProduct(other.v);
	Vector v2 = Vector(this->w * other.v.x, this->w * other.v.y, this->w * other.v.z);
	Vector v3 = Vector(other.w * this->v.x, other.w * this->v.y, other.w * this->v.z);
	Vector vTot;
...
MyQuat MyQuat::getConjugate(void) const
{
	MyQuat result;
	result.w = this->w;
	result.v = Vector(-this->v.x, -this->v.y, -this->v.z);
